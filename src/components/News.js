...
export class News extends Component {
    ...
    async componentDidMount(){
        let url = `https://newsapi.org/v2/top-headlines?country=in&apiKey=e512b5a4501144888a0353c9e884af64&Page=1&pageSize=${this.props.pageSize}`
    ...
    prevClick = async() =>{
        let url = `https://newsapi.org/v2/top-headlines?country=in&apiKey=e512b5a4501144888a0353c9e884af64&Page=${this.state.page - 1}&pageSize=${this.props.pageSize}`;
        let data = await fetch(url);
        let parsedData = await data.json()
                ...
                <div className="row">
                    {this.state.articles.map((element) => {
                        return <div className="col-md-4" key={element.url} >
                            <NewsItem title={element.title?element.title.slice(0,45):""} description={element.description?element.description.slice(0,90):"No description available at this" + "....."} imageUrl={element.urlToImage?element.urlToImage:"https://placehold.co/600x400?text=Image+Not+Available"} newsUrl={element.url} />
                        </div>
                    })}

                </div>

                <div className="container d-flex jusstify-content-between">
                    <button disabled={this.state.page <= 1} type="button" className="btn" onClick={this.prevClick} >Previous</button>
                    <button className="btn" type="button" onClick={this.nextClick} >Next</button>
                </div>
            </div>
        )
    }
